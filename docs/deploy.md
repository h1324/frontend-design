...
```bash
docker compose up -d --build     # build + start (migrations run automatically on app start)
docker compose logs -f app       # watch the app (migration output appears at startup)
docker compose ps                # health of db / app / sweeper
docker compose down              # stop (data persists in the epe_pgdata volume)
```
